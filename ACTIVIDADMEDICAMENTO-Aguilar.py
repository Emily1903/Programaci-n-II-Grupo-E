...
#Función para guardar en un archivo los datos ingresados
...
#Cardar al archivo creado
def cargar_desde_archivo_medicamentos():
    try:
        with open("medicamento.txt", "r", encoding="utf-8") as archivoMedicamento:
            medicamento_data.clear()
            for linea in archivoMedicamento:
                datos_medicamento = linea.strip().split("|")
                if len(datos_medicamento) == 4:
                    medicamento = {
                        "Nombre": datos_medicamento[0],
                        "Presentación": datos_medicamento[1],
                        "Dosis": datos_medicamento[2],
                        "Fecha Vencimiento": datos_medicamento[3],
                    }
...
def registrarMedicamento():
    ...
    #Agregar madicamento a la lista
    medicamento_data.append(medicamentos)
    guardar_en_archivo_medicamentos()
    cargar_treeview()
    #Cargar Treeview
def cargar_treeview():
    #Limpiar el Treeview
    for medicamento_registrado in treeview.get_children():
        treeview.delete(medicamento_registrado)
...
def eliminarMedicamento():
    ...
    if seleccionado:
        ...
        if messagebox.askyesno("Eliminar Medicamento",f"¿Está seguro de eliminar el medicamento {treeview.item(id_item,'values')[0]}'?"):
            ...
            guardar_en_archivo_medicamentos() #Guardar los cambios en el archivo
            cargar_treeview()
...
#Cargar los datos desde el archvo al iniciar aplicación
