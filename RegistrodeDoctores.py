...
def guardar_en_archivo_doctores():
    with open("doctorregistro.txt","w",encoding="utf-8") as archivo:
...
def cargar_desde_archivo_doctores():
    try:
        with open("doctorregistro.txt", "r", encoding="utf-8") as archivo1:
            doctores_data.clear()
        ...
        open("doctorregistro.txt", "w", encoding="utf-8").close()
...
comboCentroMedico=ttk.Combobox(ventana_principal,values=["Hospital Central","Hospital Norte","Clinica Santa Maía","Clínica Vida"],textvariable=hospital)
